...
def parse_office(file_path: str):
    ...
    with logfire.span("📄 Office Document Parsing", filename=file_path):
        try:
            # Unstructured automatically detects if it's docx or pptx and extracts content from it including text,tables,lists etc
            elements = partition(filename=file_path)
            full_text = "\n".join([str(el) for el in elements])#loop elements and combine with new line
